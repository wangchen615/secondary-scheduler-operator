...
func getPromInfo(kClient kubernetes.Interface, osrClient openshiftrouteclientset.Interface) (string, string, error) {
	rclient := osrClient.RouteV1().Routes(PromNamespace)
	...
	allTokens, err := kClient.CoreV1().Secrets(PromNamespace).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
